...
class FCCLIP(nn.Module):
    ...
    def from_config(cls, cfg):
        ...
        if deep_supervision:
            ...
            aux_weight_dict = {}
            # for i in range(dec_layers - 1): inače, ali sad mora bit dec_layers jer imaš CAL dodan
    ...
    def forward(self, batched_inputs):
        ...
        #fiksaj format dense clipova
        clip_features = features["clip_vis_dense"]
        clip_features = self.backbone.clip_model.visual.trunk.head.norm(clip_features)
        clip_features = self.backbone.clip_model.visual.head(clip_features.permute(0,2,3,1)).permute(0,3,1,2)

        features["clip_fixed"] = clip_features
