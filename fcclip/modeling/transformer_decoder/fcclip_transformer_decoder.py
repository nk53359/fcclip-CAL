...
from .transformer import TransformerDecoder,TransformerDecoderLayerLinear, TransformerDecoderLinear, TransformerDecoderLayer
...
class MultiScaleMaskedTransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            num_decoder_layer: int, number of decoder layers in the cal module 
            decoder_mask_feature: bool, use the special mask/my attention instead of normal attention
        """
        super().__init__()
        ...
        #CAL
        # learnable param 3D NxHxW za heatmap (uzeto sam iz deopa, ime je unique), hardcodano jer ne vidim parametar kako da vucem
        
        clip_embedding_dim = 768
        self.query_embedding = nn.Embedding(num_queries, hidden_dim)

        N_clip_steps = hidden_dim // 2
        self.pe_layer_heatmap = PositionEmbeddingSine(N_clip_steps, normalize=True)

        # d_model = clipF_embedding_dim #ako clip
        d_model = hidden_dim
        nhead = 8
        dim_feedforward = clip_embedding_dim * 2 #1024 naizgled treba bit samo 2put veci (to je u linearnom djelu)
        dropout = 0.1
        activation = "relu"
        normalize_before = False
        num_decoder_layer = num_decoder_layer[0]
    ...
    def from_config(cls, cfg, in_channels, mask_classification):
        ...
        ret["num_decoder_layer"] = cfg.MODEL.FC_CLIP.NUM_DECODER_LAYER,
        ret["decoder_mask_feature"] = cfg.MODEL.FC_CLIP.DECODER_MASK_FEATURE
        return ret

    def forward(self, x, mask_features, mask = None, text_classifier=None, num_templates=None, clip_features = None):
        ...
        # QxNxC        N je batchsize jer nisu normalni, Q broj maski, C dimenzija u koju kodiramo
        query_embed = self.query_embed.weight.unsqueeze(1).repeat(1, bs, 1)
        output = self.query_feat.weight.unsqueeze(1).repeat(1, bs, 1) #ovo si ti oni queriji za učenje
        ...
        for i in range(self.num_layers):
            ...
            outputs_class, outputs_mask, attn_mask = self.forward_prediction_heads(output, mask_features, attn_mask_target_size=size_list[(i + 1) % self.num_feature_levels],
                                                                                   text_classifier=text_classifier, num_templates=num_templates)
            predictions_class.append(outputs_class)
            predictions_mask.append(outputs_mask) #ovo NIJE sigmoidano koliko kužim, pazi na to
        ...
        #ovdje odma klonirat a ne na kraju isuse jedan blesavi  .detach()
        semseg_pred_mask = predictions_mask[-1]  #.clone()
        #print(predictions_mask[-1].shape) #torch.Size([1, 250, 256, 512])


        bs, h, w =mask_features.shape[0], mask_features.shape[2],mask_features.shape[3]
        
        scaleRatio = 1
        semseg_mask_downsample = F.interpolate(semseg_pred_mask, size=(h*scaleRatio,w*scaleRatio), mode="bilinear", align_corners=False)
        # 2022-11-13 添加
        if False: #self.mask_downsample:
            outputs["pred_masks"] = semseg_mask_downsample
        decoder_query_position = self.query_embedding.weight.unsqueeze(1).repeat(1, bs, 1)     
        
        pos_embed = self.pe_layer_heatmap(mask_features)
        pos_embed = pos_embed.flatten(2).permute(2,0,1)
        
        #print(decoder_query_position.shape)  torch.Size([250, 1, 256])
        #print(pos_embed.shape)  torch.Size([2048, 1, 256])
        #print(semseg_mask_downsample.shape) torch.Size([1, 250, 32, 64])

        tgt = torch.zeros_like(decoder_query_position)
        mask = None
        
        memory = mask_features
        memory = memory.flatten(2).permute(2,0,1)
        """
        print(tgt.shape) torch.Size([250, 1, 256])
        print(memory.shape) torch.Size([2048, 1, 768])
        print(pos_embed.shape) torch.Size([2048, 1, 256])
        print(decoder_query_position.shape) torch.Size([250, 1, 256])
        """
        hs = self.decoder(
        ...
        hs = hs.transpose(1,2)
        #torch.Size([2, 1, 250, 256])
        outputs_class_embed = hs[-1]

        #outputs["pred_logits"] = outputs_class[-1] #ovdje su sada BxNxCemb size heatmap vektori
        

        class_embed = self.class_embed(outputs_class_embed)
        #torch.Size([1, 250, 134])
        outputs_class_embed = get_classification_logits(class_embed, text_classifier, self.logit_scale, num_templates)
        
        predictions_class.append(outputs_class_embed)
        predictions_mask.append(semseg_pred_mask.clone()) #.clone()
        """
        ...
        print(self.mask_classification)
        print(x.shape)
        """
        return out
    ...
    def forward_prediction_heads(self, output, mask_features, attn_mask_target_size, text_classifier, num_templates):
        ...
        """
        print(outputs_mask.shape)
        print(maskpool_embeddings.shape)
        print(class_embed.shape)
        torch.Size([1, 250, 256, 512])
